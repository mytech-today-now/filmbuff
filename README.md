...
[![Version](https://img.shields.io/badge/version-2.5.6-blue.svg)](https://github.com/mytech-today-now/filmbuff)
...
Module-maintenance commands are also available for linked modules: `filmbuff update`, `filmbuff upgrade <module>`, `filmbuff pin <module> <version>`, `filmbuff version-info <module> [--json]`, and `filmbuff check-updates`. Workspace recovery lives under `filmbuff lifecycle` with `status`, `install`, `reinstall`, `upgrade`, `diagnose`, `recover`, `repair`, `rollback`, and `uninstall` subcommands.

When you use `filmbuff version-info --json`, alias lookups resolve to the canonical module id in the JSON payload so machine-readable output stays stable.
