...
# ─── PHASE 7: Credential Security ────────────────────────────────────────────

Write-Step "Phase 7: Credential Security (agentToken audit - AC-27)"

$ph7Desc = @"
PHASE: Credential Security (implementation.md Security Architecture + design.md)
BLOCKED BY: Phase 6 (MCP server adds new output paths that must be audited)
BLOCKS: Phase 8 (testing must verify the security invariant passes)

TASKS (from tasks.md Phase 7):
  • Audit ALL code paths to confirm agentToken is not logged, printed, or written to any file
  • Paths to audit: StatusFileManager, ShotStateMachine, all Renderers, all log paths,
    stdout, stderr, 08-video-status.jsonl, MCP response bodies, any batch output files
  • Add lint rule or test assertion that scans all serialized output to confirm token is absent
  • Verify AC-27: AIPOWERED_AGENT_TOKEN never logged, printed to stdout/stderr, or written to file

SECURITY ARCHITECTURE (design.md § Security Architecture):
  agentToken flows: ENV var → SingleShotOptions.agentToken → HTTP Authorization: Bearer ONLY
  Token injection boundary: ProviderIntegration layer (ai-powered wrapper)
  Token MUST NOT cross into: StatusFileManager, ShotStateMachine, Renderer, or any log path

CREDENTIAL PASSTHROUGH (deltas.md Domain D):
  When AIPOWERED_AGENT_TOKEN is set, filmbuff passes it as agentToken in SingleShotOptions.
  The token must NEVER appear in: status files, log files, stdout, stderr, or any output file.

SPEC REF: openspec/changes/filmb-ai-p/specs/agent-mode/spec.md § Credential Passthrough
AC LINKS: AC-27 (agentToken never logged, printed, or written to any file)
"@

$ph7Id = New-BeadTask `
    -Title       "[filmb-ai-p] Phase 7: Credential Security (agentToken audit - AC-27)" `
    -Description $ph7Desc `
    -Priority    1 `
    -Type        "task" `
    -Labels      @("filmb-ai-p","phase-7","security","credential-audit","AC-27")
